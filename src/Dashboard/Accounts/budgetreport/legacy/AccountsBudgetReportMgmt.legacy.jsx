...
const ACCOUNT_PERCENT = 0.8; // 80% of event account amount ΓÇô max we can allocate as "account"

const formatINR = (val) => {
  if (val == null || isNaN(Number(val))) return "ΓÇö";
  return `Γé╣${Number(val).toLocaleString("en-IN", {
      ...
      Object.entries(groups).forEach(([groupName, rows]) => {
        (rows || []).forEach((row, indexInGroup) => {
          ...
          built.push({
            ...
            vendorName: row.vendorName || row.vendorCode || "ΓÇö",
  ...
  if (loading) {
    return (
      <div className="budget-report-container flex items-center justify-center min-h-[50vh]">
        <Spin size="large" tip="LoadingΓÇª" />
      </div>
    );
  }
